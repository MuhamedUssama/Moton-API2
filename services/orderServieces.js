...
exports.checkoutSession = asyncHandler(async (req, res, next) => {
  ...
  if (paymentGetaway === "paymob") {
    ...
    const paymobOrderId = await createOrder(
      token,
      totalOrderPriceAsCents,
      `${cart._id.toString()} ${Date.now()}`
    );
  ...
  if (paymentGetaway === "paypal") {
    const paymentUrl = await createPaypalPayment(
      totalOrderPrice,
      "USD",
      cart._id
    );
...
exports.createPaymobOrder = asyncHandler(async (req, res, next) => {
  ...
  const { isSuccess, amount_cents } = await checkIfOrderPaid(
    token,
    transactionId
  );
  if (!isSuccess) {
